...
class Crypto implements ICrypto {
  ...
  KeypairDetails generateKeypair(String passphrase, String pinCode, String salt) {
    ...
    if (status != TagionErrorCode.none.value) {
      ...
      _pointerManager.free(securenetPtr);
    ...
    /// Get the values.
    final secureNet = securenetPtr.ref;
  ...
  Uint8List sign(SecureNet keypair, Uint8List dataToSign) {
    ...
    if (status != TagionErrorCode.none.value) {
      /// Free memory.
      _pointerManager.free(keypairPtr);
      _pointerManager.zeroOutAndFree(dataToSignPtr, dataToSignLen);
